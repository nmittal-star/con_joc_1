...
import { ProductsDataService } from '../../../data-access/product/products/products.api';
...
export class Products {
  ...
  loadProducts(
    ...
    this.productsDataService.getProducts().pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        const fullData = res
  ...
  onSortChange(sort: Sort) {
    this.loadProducts(1, this.pageSize, this.currentSearchTerm, sort);
  }

  onPageChange(page: number) {
    this.loadProducts(page, this.pageSize, this.currentSearchTerm, this.currentSort);
  }

  onPageSizeChange(size: number) {
    this.loadProducts(1, size, this.currentSearchTerm, this.currentSort);
  }

  onSearch(term: string) {
    this.loadProducts(1, this.pageSize, term, this.currentSort);
  }
