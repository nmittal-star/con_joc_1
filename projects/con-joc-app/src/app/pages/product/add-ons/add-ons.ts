...
import { AddonsDataService } from '../../../data-access/product/add-ons/add-ons.api';
...
export class AddOns {
  ...
  constructor(private addonsDataService: AddonsDataService) { }
  ngOnInit() {
    this.loadAddons()
  ...
  loadAddons(
    ...
    this.addonsDataService.getAddons().pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        const fullData = res
      ...
      complete: () => {
        this.addonsConfig.loading = false;
      }
    });
  ...
  onSortChange(sort: Sort) {
    this.loadAddons(1, this.pageSize, this.currentSearchTerm, sort);
  }

  onPageChange(page: number) {
    this.loadAddons(page, this.pageSize, this.currentSearchTerm, this.currentSort);
  }

  onPageSizeChange(size: number) {
    this.loadAddons(1, size, this.currentSearchTerm, this.currentSort);
  }

  onSearch(term: string) {
    this.loadAddons(1, this.pageSize, term, this.currentSort);
  }
