...
import { FeaturesDataService } from '../../../data-access/product/features/features.api';
...
export class Features {
  ...
  loadFeatures(
    ...
    this.featuresDataService.getFeatures().pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        const fullData = res
      ...
      complete: () => {
        this.featuresConfig.loading = false;
      }
    });
  ...
  onSortChange(sort: Sort) {
    this.loadFeatures(1, this.pageSize, this.currentSearchTerm, sort);
  }

  onPageChange(page: number) {
    this.loadFeatures(page, this.pageSize, this.currentSearchTerm, this.currentSort);
  }

  onPageSizeChange(size: number) {
    this.loadFeatures(1, size, this.currentSearchTerm, this.currentSort);
  }

  onSearch(term: string) {
    this.loadFeatures(1, this.pageSize, term, this.currentSort);
  }
