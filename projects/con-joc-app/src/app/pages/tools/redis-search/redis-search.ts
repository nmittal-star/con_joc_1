...
import { RedisDataService } from '../../../data-access/tools/redis-search/redis-search.api';

@Component({
  selector: 'app-redis-search',
    standalone: true,
  imports: [CommonModule, TableComponent, ReactiveFormsModule, FormsModule, MatIconModule],
  templateUrl: './redis-search.html',
  styleUrl: './redis-search.scss',
})
export class RedisSearch  {
  ...
  readonly columns: TableColumn[] = [
    ...
    { key: 'time', label: 'Time', sortable: true, searchable: true },
    { key: 'data', label: 'Data', sortable: true, searchable: true },
  ...
  loadExtensions(
    ...
    this.redisDataService.getRedis().pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        const fullData = res
      ...
      complete: () => {
        this.redissearchConfig.loading = false;
      }
    });
