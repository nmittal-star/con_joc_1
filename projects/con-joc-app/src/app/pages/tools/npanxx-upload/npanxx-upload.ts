...
import { ButtonType, DropAreaComponent, DropAreaConfig, PaginationConfig, TableColumn, TableComponent, TableConfig, TableFilterConfig, UserData } from '@eh-library/common';
import { BehaviorSubject, Observable, Subject, takeUntil } from 'rxjs';
import { NpanxxUploadDataService } from '../../../data-access/tools/npanxx-upload/npanxx-upload.api';
@Component({
  selector: 'app-npanxx-upload',
  standalone: true,
  imports: [CommonModule, TableComponent, ReactiveFormsModule, FormsModule, MatIconModule,DropAreaComponent],
  templateUrl: './npanxx-upload.html',
  styleUrl: './npanxx-upload.scss',
})
export class NpanxxUpload {
  ...
  readonly columns: TableColumn[] = [
    ...
    { key: 'job', label: 'Job', sortable: true, searchable: true },
    { key: 'file', label: 'File', sortable: true, searchable: true },
    { key: 'totalRows', label: 'Total Rows', sortable: true, searchable: true },
    { key: 'startedAt', label: 'Started At', sortable: true, searchable: true },
    { key: 'finishedAt', label: 'Finished At', sortable: true, searchable: true },
    { key: 'status', label: 'Status', sortable: true, searchable: true },
    { key: 'inProdRowsCount', label: 'In-Prod Rows Count', sortable: true, searchable: true },
    { key: 'inStageRowsCount', label: 'In-Stage Rows Count', sortable: true, searchable: true },
  ...
  constructor(private npanxxUploadDataService: NpanxxUploadDataService) { }
  ngOnInit() {
  ...
  loadStateTracker(
    ...
    this.npanxxUploadDataService.getNpanxxUpload().pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        const fullData = res
