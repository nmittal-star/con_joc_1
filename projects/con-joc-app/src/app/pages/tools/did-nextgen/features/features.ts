...
import { FeatureSettingsDataService } from '../../../../data-access/tools/did-nextgen/features/features.api';
...
export class Features {
  ...
  featuresettingsConfig: TableConfig = {
    showSearch: true,
    showExport: false,
  ...
  readonly columns: TableColumn[] = [
    ...
    { key: 'settings', label: 'Settings', sortable: true, searchable: true },
  ...
  constructor(private featuresettingsDataService: FeatureSettingsDataService) { }
  ngOnInit() {
    this.loadFeature()
  ...
  loadFeature(
    ...
    this.featuresettingsDataService.getFeatures().pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        const fullData = res
      ...
      complete: () => {
        this.featuresettingsConfig.loading = false;
      }
    });
  ...
  onSortChange(sort: Sort) {
    this.loadFeature(1, this.pageSize, sort);
  }

  onPageChange(page: number) {
    this.loadFeature(page, this.pageSize, this.currentSort);
  }

  onPageSizeChange(size: number) {
    this.loadFeature(1, size, this.currentSort);
  }
