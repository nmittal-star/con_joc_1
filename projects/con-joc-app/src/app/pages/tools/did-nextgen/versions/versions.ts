...
import { VersionsDataService } from '../../../../data-access/tools/did-nextgen/versions/versions .api';
...
export class Versions {
  ...
  readonly columns: TableColumn[] = [
    ...
    { key: 'version', label: 'version', sortable: true, searchable: true },
    { key: 'name', label: 'Name', sortable: true, searchable: true },
    { key: 'description', label: 'Description', searchable: true },
    { key: 'features', label: "features", sortable: true, searchable: true },
  ...
  constructor(private versionsDataService: VersionsDataService) { }
  ngOnInit() {
    this.loadVersions()
  ...
  loadVersions(
    ...
    this.versionsDataService.getVersions().pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        const fullData = res
      ...
      complete: () => {
        this.versionConfig.loading = false;
      }
    });
  ...
  onSortChange(sort: Sort) {
    this.loadVersions(1, this.pageSize, sort);
  }

  onPageChange(page: number) {
    this.loadVersions(page, this.pageSize, this.currentSort);
  }

  onPageSizeChange(size: number) {
    this.loadVersions(1, size, this.currentSort);
  }
