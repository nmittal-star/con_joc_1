...
import { DispositionDataService } from '../../../data-access/tools/disposition-delays/disposition-delays.api';

@Component({
  selector: 'app-disposition-delays',
  standalone: true,
  imports: [CommonModule, TableComponent, ReactiveFormsModule, FormsModule, MatIconModule],
  templateUrl: './disposition-delays.html',
  styleUrl: './disposition-delays.scss',
})
export class DispositionDelays {
  ...
  readonly columns: TableColumn[] = [
    ...
    { key: 'dialable_status', label: 'Dial-able Status', sortable: true, searchable: true },
  ...
  constructor(private dispositionDataService: DispositionDataService) { }
  ngOnInit() {
  ...
  loadDispositions(
    ...
    this.dispositionDataService.getDispositionDelays().pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        const fullData = res
  ...
  onSortChange(sort: Sort) {
    this.loadDispositions(1, this.pageSize, this.currentSearchTerm, sort);
  }

  onPageChange(page: number) {
    this.loadDispositions(page, this.pageSize, this.currentSearchTerm, this.currentSort);
  }

  onPageSizeChange(size: number) {
    this.loadDispositions(1, size, this.currentSearchTerm, this.currentSort);
  }

  onSearch(term: string) {
    this.loadDispositions(1, this.pageSize, term, this.currentSort);
  }
