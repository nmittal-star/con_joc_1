...
import { MetabaseReportsDataService } from '../../../data-access/reports/metabase-reports/metabase-reports.api';

@Component({
  selector: 'app-metabase-reports',
  standalone:true,
  imports: [CommonModule, TableComponent, ReactiveFormsModule, FormsModule, MatIconModule],
  templateUrl: './metabase-reports.html',
  styleUrl: './metabase-reports.scss',
})
export class MetabaseReports {
  ...
  currentSort: Sort = { active: '', direction: '' };
    ...
    metabasereportsConfig: TableConfig = {
    showSearch: true,
    showExport: false,
    ...
    { key: 'createdDate', label: 'Created Date', sortable: true, searchable: true },
    { key: 'publishedDate', label: 'Published Date', sortable: true, searchable: true },
    { key: 'publishedStatus', label: 'Published Status', searchable: true },
    {
      key: 'actions',
      label: 'Actions',
      type: 'action',
    ...
    constructor(private metabaseReportsDataService:MetabaseReportsDataService){}
    ngOnInit(){
      this.loadMetabaseReports()
  ...
  ngOnDestroy(): void {
   ...
   loadMetabaseReports(
    ...
    this.metabaseReportsDataService.getMetabaseReports().pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        const fullData = res
      ...
      complete: () => {
        this.metabasereportsConfig.loading = false;
      }
    });
    ...
    this.loadMetabaseReports(1, this.pageSize,  sort);
  }

  onPageChange(page: number) {
    this.loadMetabaseReports(page, this.pageSize,  this.currentSort);
  }

  onPageSizeChange(size: number) {
    this.loadMetabaseReports(1, size,  this.currentSort);
  }
