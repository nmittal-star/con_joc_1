...
import { TransactionsDataService } from '../../../data-access/omni/transactions/transactions.api';
...
export class Transactions {
   private dataSubject = new BehaviorSubject<UserData[]>([]);
      ...
      currentSort: Sort = { active: '', direction: '' };
        ...
        transactionsConfig: TableConfig = {
        showSearch: true,
        showExport: false,
        ...
        { key: 'creditCard', label: 'Credit Card', sortable: true, searchable: true },
        { key: 'transactionid', label: 'Transaction ID', sortable: true, searchable: true },
        { key: 'subscriptionId', label: 'Subscription ID', sortable: true, searchable: true },
        { key: 'accountId', label: 'Account ID', sortable: true, searchable: true },
        { key: 'memo', label: 'Memo', sortable: true, searchable: true },
        {
          key: 'actions',
          label: 'Actions',
          type: 'action',
          ...
          actions: [
            ...
            { icon: 'edit', tooltip: 'Settings', callback: (row) => this.editSettngs(row) },
      ...
      ngOnDestroy(): void {
       ...
       loadTransactions(
        ...
        this.transactionsDataService.getTransactions().pipe(takeUntil(this.destroy$)).subscribe({
          next: (res: any) => {
            const fullData = res
          ...
          complete: () => {
            this.transactionsConfig.loading = false;
          }
        });
        ...
        this.loadTransactions(1, this.pageSize, this.currentSearchTerm, sort);
      }
    
      onPageChange(page: number) {
        this.loadTransactions(page, this.pageSize, this.currentSearchTerm, this.currentSort);
      }
    
      onPageSizeChange(size: number) {
        this.loadTransactions(1, size, this.currentSearchTerm, this.currentSort);
      }
    
      onSearch(term: string) {
        this.loadTransactions(1, this.pageSize, term, this.currentSort);
      }
