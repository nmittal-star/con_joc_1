...
@Component({
  selector: 'app-view-didorders',
  standalone: true,
  imports: [CommonModule, TableComponent, ReactiveFormsModule, FormsModule, MatIconModule],
  templateUrl: './view-didorders.html',
  styleUrl: './view-didorders.scss',
})
export class ViewDidorders {
  ...
  viewdidordersConfig: TableConfig = {
    showSearch: true,
    showExport: false,
  ...
  readonly columns: TableColumn[] = [
    ...
    { key: 'isPaid', label: 'IS Paid', sortable: true, searchable: true },
    ...
    { key: 'error', label: 'Error', sortable: true, searchable: true },
    { key: 'replacedWith', label: 'Replaced With', sortable: true, searchable: true },
    { key: 'lastReplaceAttempt', label: 'LAST REPLACE ATTEMPT', sortable: true, searchable: true },
    {
      key: 'actions',
      label: 'Actions',
      type: 'action',
      ...
      actions: [
         ...
         { icon: 'autorenew', tooltip: 'Replace', callback: (row) => this.onReplace(row) },
         { icon: 'refresh', tooltip: 'Retry', callback: (row) => this.onRetry(row) },
         { icon: 'delete', tooltip: 'Remove', callback: (row) => this.onRemove(row) },
      ],
    },
  ];

  constructor(private viewdidOrdersDataService: DIDOrdersDataService) { }
  ngOnInit() {
    this.loadViewDIDOrders()
  ...
  loadViewDIDOrders(
    ...
    this.viewdidOrdersDataService.getViewOrders().pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        const fullData = res
      ...
      complete: () => {
        this.viewdidordersConfig.loading = false;
      }
    });
  ...
  onSortChange(sort: Sort) {
    this.loadViewDIDOrders(1, this.pageSize, this.currentSearchTerm, sort);
  }

  onPageChange(page: number) {
    this.loadViewDIDOrders(page, this.pageSize, this.currentSearchTerm, this.currentSort);
  }

  onPageSizeChange(size: number) {
    this.loadViewDIDOrders(1, size, this.currentSearchTerm, this.currentSort);
  }

  onSearch(term: string) {
    this.loadViewDIDOrders(1, this.pageSize, term, this.currentSort);
  }
