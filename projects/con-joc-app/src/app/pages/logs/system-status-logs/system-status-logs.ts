...
@Component({
  ...
  templateUrl: './system-status-logs.html',
  styleUrl: './system-status-logs.scss',
})
export class SystemStatusLogs{
  ...
  currentSort: Sort = { active: '', direction: '' };
    ...
    systemstatuslogConfig: TableConfig = {
    showSearch: true,
    showExport: false,
    ...
    { key: 'reason', label: 'Reason For Outage', sortable: true, searchable: true },
    { key: 'tickets', label: 'Tickets', sortable: true, searchable: true },
    { key: 'suggestions', label: 'Suggestions', searchable: true },
  ...
  ngOnDestroy(): void {
   ...
   loadSystemStatusLogs(
    ...
    this.systemStatusLogsDataService.getSystemStatusLogs().pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        const fullData = res
      ...
      complete: () => {
        this.systemstatuslogConfig.loading = false;
      }
    });
  ...
  onPageSizeChange(size: number) {
    this.loadSystemStatusLogs(1, size, this.currentSearchTerm, this.currentSort);
  }

  onSearch(term: string) {
    this.loadSystemStatusLogs(1, this.pageSize, term, this.currentSort);
  }
