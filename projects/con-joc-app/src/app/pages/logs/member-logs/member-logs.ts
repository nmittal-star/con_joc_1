...
import { MemberLogsDataService } from '../../../data-access/logs/member-logs/member-logs.api';

@Component({
  selector: 'app-member-logs',
  standalone:true,
  imports: [CommonModule, TableComponent, ReactiveFormsModule, FormsModule, MatIconModule],
  templateUrl: './member-logs.html',
  styleUrl: './member-logs.scss',
})
export class MemberLogs {
  ...
  currentSort: Sort = { active: '', direction: '' };
   ...
   advFilterConfig: TableFilterConfig = {
    ...
    { key: 'ip_adress', label: 'IP Address', sortable: true, searchable: true },
    { key: 'section', label: 'Section', sortable: true, searchable: true },
    { key: 'subsection', label: 'Subsection', sortable: true, searchable: true },
    { key: 'modifiedby', label: 'Modified By', searchable: true },
    { key: 'id', label: 'Unique ID', sortable: true, searchable: true },
    { key: 'difference', label: 'Difference', sortable: true, searchable: true },
    ...
    constructor(private memberLogsDataService:MemberLogsDataService){}
    ngOnInit(){
      this.loadMemberLogs()
  ...
  ngOnDestroy(): void {
   ...
   loadMemberLogs(
    ...
    this.memberLogsDataService.getMemberLogs().pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        const fullData = res
      ...
      complete: () => {
        this.memberslogConfig.loading = false;
      }
    });
    ...
    this.loadMemberLogs(1, this.pageSize, this.currentSearchTerm, sort);
  }

  onPageChange(page: number) {
    this.loadMemberLogs(page, this.pageSize, this.currentSearchTerm, this.currentSort);
  }

  onPageSizeChange(size: number) {
    this.loadMemberLogs(1, size, this.currentSearchTerm, this.currentSort);
  }

  onSearch(term: string) {
    this.loadMemberLogs(1, this.pageSize, term, this.currentSort);
  }
